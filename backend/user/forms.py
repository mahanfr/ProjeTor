...
from user.validators import validate_phone_number, validate_username
...
class UserRegisterForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).count() > 0:
            raise forms.ValidationError(
                self.error_messages["email_in_use"], code="email_in_use")
        return email
    ...
    def clean_user_name(self):
        ...
        if User.objects.filter(user_name=user_name).count() > 0:
            raise forms.ValidationError(
                self.error_messages["user_name_in_use"], code="user_name_in_use")
        return user_name
    ...
    def clean_phone_number(self):
        ...
        if User.objects.filter(phone_number=phone_number).count() > 0:
            raise forms.ValidationError(
                self.error_messages["phone_number_in_use"], code="phone_number_in_use")
        return phone_number
